...
[B0_m1 Del0_m1 Miu_m1 Del_m1 Eps_m1 SigRsurf_m1 SigIsurf_m1 SigItb_m1 mSig_m1 Eta_m1 F_m1] = fLoadDataFromFile('MN_005.txt');
[B0_m2 Del0_m2 Miu_m2 Del_m2 Eps_m2 SigRsurf_m2 SigIsurf_m2 SigItb_m2 mSig_m2 Eta_m2 F_m2] = fLoadDataFromFile('MN_015.txt');
[B0_m3 Del0_m3 Miu_m3 Del_m3 Eps_m3 SigRsurf_m3 SigIsurf_m3 SigItb_m3 mSig_m3 Eta_m3 F_m3] = fLoadDataFromFile('MN_025.txt');
[B0_m Del0_m Miu_m Del_m Eps_m SigRsurf_m SigIsurf_m SigItb_m mSig_m Eta_m F_m] = fLoadDataFromFile('MN.txt');
...
legend({'Данные','Аппроксимация'},'FontSize',12);
grid on;
grid minor;
...
%% 6///////
for i=1:N+1
    mSig1(i) = fmSig(Miu1, X(i));
    mSig2(i) = fmSig(Miu2, X(i));
    mSig3(i) = fmSig(Miu3, X(i));
end

figure(6);
scatter(X_m,mSig_m1,60,'^','filled');
hold on;
plot(X,mSig1,'LineWidth',1.5);
hold on;
scatter(X_m,mSig_m2,60,'s','filled');
hold on;
plot(X,mSig2,'LineWidth',1.5);
hold on;
scatter(X_m,mSig_m3,60,'d','filled');
hold on;
plot(X,mSig3,'LineWidth',1.5);
ax = gca;
ax.XAxis.FontSize = 12;
ax.YAxis.FontSize = 12;
xlabel('$1000\frac{\Delta_{0}}{B_{0}} [-]$','Interpreter','latex','FontSize',16);
ylabel('m_{\sigma} [-]','FontSize',16);
legend({'Данные при \mu=0,05','Аппроксимация при \mu=0,05','Данные при \mu=0,15','Аппроксимация при \mu=0,15','Данные при \mu=0,25','Аппроксимация при \mu=0,25'},'FontSize',12);
grid on;
grid minor;
...
figure(8);
scatter(Eps_m1,F_kNmm_m1,60,'^','filled');
hold on;
plot(Eps,F_kNmm1,'LineWidth',1.5);
hold on;
scatter(Eps_m2,F_kNmm_m2,60,'s','filled');
hold on;
plot(Eps,F_kNmm2,'LineWidth',1.5);
hold on;
scatter(Eps_m3,F_kNmm_m3,60,'d','filled');
hold on;
plot(Eps,F_kNmm3,'LineWidth',1.5);
ax = gca;
ax.XAxis.FontSize = 12;
ax.YAxis.FontSize = 12;
xlabel('\epsilon [%]','FontSize',16);
ylabel('F [кН/мм]','FontSize',16);
legend({'Данные при \mu=0,05','Аппроксимация при \mu=0,05','Данные при \mu=0,15','Аппроксимация при \mu=0,15','Данные при \mu=0,25','Аппроксимация при \mu=0,25'},'FontSize',12);
grid on;
grid minor;
...
function mSig = fmSig(Miu, X)
    a1=2.15501;
    a2=0.000139;
    a3=0.01089;
    a4=0.47363;
%     X=1000*Del0/B0;
    mSig=(a1+a2/Miu)*(a3*X+a4);
end
